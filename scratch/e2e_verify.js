(async () => {
  ...
  // Load connection string from db config (primaryUri)
  const dbConfig = require('./backend/config/db');
  // The config file exports a function that attempts connections; we will extract the URI manually
  // Since the file defines primaryUri locally, we replicate the logic to get the actual URI used.
  const envUri = process.env.MONGODB_URI;
  let primaryUri = "mongodb+srv://sanaullaa19_db_user:Suhail%4008@ac-hakzppl-shard-00-01.pzskqem.mongodb.net/mefy_production?retryWrites=true&w=majority";
  let urisToTry = [];
  if (envUri) {
  ...
  // If created, delete test appointment
